...
if __name__=='__main__':
    ...
    result_img_dir='./detectron/datasets/data/kaggle_ship/train/rle_vis_result/'#可视化标签保存的文件夹
    ...
    for csv_mask in csv_masks.itertuples():
        ...
        ##--------------------------------------------------------------
        ##因为Detectron只能使用poly就不算rle了，节省时间
        # cur_rle=mask2K_rle(cur_mask)
        ##--------------------------------------------------------------
        #可视化gt
        raw_img=mpimg.imread(raw_img_path+img_name)
        seg_list,cur_contours=mask2poly(cur_mask)
        raw_img = cv2.drawContours(raw_img, cur_contours, -1, (0,255,0), 3)
        ...
        for region in measure.regionprops(cur_mask): 
            ...
            ax1.add_patch(rect)
        #保存gt结果，左边是mask，右边是bbox和poly画在原图上
        plt.savefig(result_img_dir+img_name)
        #-----------------------------------------------------------------------------------------
        cur_img_dict={}
